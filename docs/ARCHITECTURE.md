...
- `custom_fields.json` は管理者権限が必要 → 失敗しても他は動くよう非致命的に扱う
...
- **`custom_fields.json` は要管理者権限**：非 admin キーだと CF 一覧が取れない。
  その場合も他ツールは動くよう、失敗を非致命的に扱う設計を壊さないこと。
...
- `custom_fields.json` / `users.json` 等の一部エンドポイントは管理者権限が必要
- 書き込み操作は非対応（設計上の意図）
