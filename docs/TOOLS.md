...
大量プロジェクトを抱える Redmine で「自分の担当プロジェクト群だけ集計したい」用途に最適。
...
| `tracker` | string | — | 全トラッカー | トラッカー名。例: `"バグ"`, `"機能"`, `"タスク"` |
  ...
  "issues": [
    {
      ...
      "subject": "ログイン画面でセッションが切れると500エラーになる",
      "project": "TestRedmine",
      "tracker": "バグ",
      "status": "完了",
      "priority": "低カスタム",
...
```jsonc
{
  ...
  "id": 3,
  "subject": "ログイン処理のセッション期限を24時間に延長",
  "description": "OAuth セッションの有効期限を伸ばす対応。",
